...
export default function AureliaTableStationMatrix() {
  ...
  // 🏛️ आपके पैनल के 5 मुख्य लाइव स्टेशन्स
  const stations = [
  ...
  // 🏆 AUTOMATED EMBEDDED URL GRID GENERATOR (यह कभी ब्लैंक नहीं हो सकता)
  const getSecureQrSource = (id: string) => {
    const targetUrl = `https://vercel.app{id}`;
    // वैश्विक स्तर पर सबसे भरोसेमंद और सुरक्षित इन्सुलेटर लिंक जनरेशन
    return `https://quickchart.io{encodeURIComponent(targetUrl)}&size=200&margin=1&ecLevel=H`;
  };
  ...
  return (
    <div style={{ background: '#F8FAFC', minHeight: '100vh', display: 'flex', fontFamily: "'Poppins', sans-serif" }}>
      ...
      <div style={{ flex: 1, padding: '3rem', boxSizing: 'border-box', position: 'relative' }}>
        ...
        {/* 🔮 DYNAMIC POPUP MODAL: PURE SECURED RENDERING LAYERS */}
        {activeStationId && (
          <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(11, 15, 25, 0.85)', zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ background: '#0B0F19', border: '2px solid #F59E0B', borderRadius: '2.5rem', padding: '3rem', width: '360px', textAlign: 'center', boxShadow: '0 25px 50px -12px rgba(245,158,11,0.25)', position: 'relative' }}>
              ...
              {/* 🏆 SURESHOT RENDER: साधारण <img> टैग से 'unoptimized' को पूरी तरह हटा दिया गया है */}
              <div style={{ background: '#FFF', padding: '1.5rem', borderRadius: '1.5rem', display: 'inline-block', boxShadow: '0 10px 25px rgba(0,0,0,0.5)', marginBottom: '2rem' }}>
                <img
                  src={getSecureQrSource(activeStationId)}
                  alt="Aurelia Dynamic QR Grid"
              ...
              <span style={{ color: '#9CA3AF', fontSize: '0.8rem', display: 'block', marginBottom: '2rem', letterSpacing: '0.05rem' }}>
                Scan code to launch 7-Star fine dine dynamic basket router.
              </span>

              <div style={{ display: 'flex', gap: '1rem' }}>
                <button onClick={() => window.print()} style={{ flex: 1, padding: '0.8rem', background: '#F59E0B', color: '#0B0F19', border: 'none', borderRadius: '0.8rem', fontWeight: 700, cursor: 'pointer', textTransform: 'uppercase', fontSize: '0.75rem', letterSpacing: '0.05rem' }}><i className="fa-solid fa-print" style={{ marginRight: '0.4rem' }}></i> Print Card</button>
                <button onClick={() => setActiveStationId(null)} style={{ flex: 1, padding: '0.8rem', background: 'rgba(255,255,255,0.05)', color: '#FFF', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '0.8rem', fontWeight: 700, cursor: 'pointer', textTransform: 'uppercase', fontSize: '0.75rem' }}>Dismiss</button>
              </div>
